...
struct tftlcd_typedef{
    uint16_t height; /* LCD 屏幕高度 */
    uint16_t width; /* LCD 屏幕宽度 */
    uint8_t pixsize; /* LCD 每个像素所占字节大小 */
    uint16_t vspw; /* VSYNC 信号宽度 */
    uint16_t vbpd; /* 帧同步信号后肩 */
    uint16_t vfpd; /* 帧同步信号前肩 */
    uint16_t hspw; /* HSYNC 信号宽度 */
    uint16_t hbpd; /* 水平同步信号后见肩 */
    uint16_t hfpd; /* 水平同步信号前肩 */
    uint32_t framebuffer; /* LCD 显存首地址 */
...
void video_pllinit(uint8_t loopdivi, uint8_t postdivi);

inline void lcd_drawpoint(uint16_t x,uint16_t y,uint32_t color);
inline uint32_t lcd_readpoint(uint16_t x,uint16_t y);
void lcd_clear(uint32_t color);
void lcd_fill(unsigned    short x0, uint16_t y0, uint16_t x1, uint16_t y1, uint32_t color);
#endif
