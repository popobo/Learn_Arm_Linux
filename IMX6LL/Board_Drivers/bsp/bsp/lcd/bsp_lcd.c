...
uint16_t lcd_read_panelid(void)
{
	...
	idx = (uint8_t)gpio_pinread(GPIO3, 28); 	/* 读取M0 */
	idx |= (uint8_t)gpio_pinread(GPIO3, 20) << 1;	/* 读取M1 */
	idx |= (uint8_t)gpio_pinread(GPIO3, 12) << 2;	/* 读取M2 */
...
{ 
  	*(uint32_t*)((uint32_t)tftlcd_dev.framebuffer + 
...
{ 
	return *(uint32_t*)((uint32_t)tftlcd_dev.framebuffer + 
...
void lcd_clear(uint32_t color)
{
	...
	uint32_t *startaddr=(uint32_t*)tftlcd_dev.framebuffer;	//指向帧缓存首地址
	num=(uint32_t)tftlcd_dev.width * tftlcd_dev.height;			//缓冲区总长度
	for(i = 0; i < num; i++)
	{
		startaddr[i] = color;
	}		
 ...
 * @return 			: 读取到的指定点的颜色值
 */
void lcd_fill(unsigned    short x0, uint16_t y0, 
