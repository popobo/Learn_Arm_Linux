...
struct ap3216c_dev
{
    ...
    unsigned short ir, als, ps; /* 三个光传感器 */
};
...
static s32 ap3216c_write_regs(struct ap3216c_dev *dev, u8 reg, u8 *in_buf, u8 len)
{
    u8 buf[256];
    struct i2c_msg msg;
    struct i2c_client *client = (struct i2c_client*)dev->private_data;
    ...
    buf[0] = reg;                 /* 寄存器首地址 */
    memcpy(&buf[1], in_buf, len); /* 将要写入的数据拷贝到数组b里面 */
...
/*
 * @description : 读取 AP3216C 的数据，读取原始数据，包括 ALS,PS 和 IR, 同时打开 ALS,IR+PS 的话两次数据读取的间隔要大于 112.5ms
 * @param - ir : ir 数据
 * @param - ps : ps 数据
 * @param - ps : als 数据
 * @return : 无。
...
ssize_t ap3216c_read(struct file *fp, char __user *buf, size_t cnt, loff_t *off)
{
    short data[3];
    int err = 0;
    struct ap3216c_dev *dev = fp->private_data;
                         ...
                         const struct i2c_device_id *id)
{
    ...
    if (ap3216c.major)
    {
        ...
        register_chrdev_region(ap3216c.devid, AP3216C_CNT,
                               AP3216C_NAME);
    }
    else
    {
        alloc_chrdev_region(&ap3216c.devid, 0, AP3216C_CNT,
                            AP3216C_NAME);
    ...
    /* 4、创建设备 */
    ap3216c.device = device_create(ap3216c.class, NULL,
                                   ap3216c.devid, NULL, AP3216C_NAME);
